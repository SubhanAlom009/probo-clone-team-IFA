...
export async function placeOrder({ eventId, userId, side, price, quantity }) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    const matchPrice = side === "yes" ? 10 - price : 10 - price;
    ...
    for (const opp of oppOrders) {
      ...
      // Prevent self-trading: skip if user is matching their own order
...
export async function resolveEvent(eventId, outcome, resolverId) {
  ...
  // Refund truly unmatched open orders (status 'open' and quantityRemaining > 0)
  const ordersQ = query(
  ...
  for (const order of orderSnaps.docs.map((d) => ({ id: d.id, ...d.data() }))) {
    // Only refund if still unmatched and not involved in any bet
    if (!order.lockedAmount || order.lockedAmount <= 0) continue;
    if (!order.quantityRemaining || order.quantityRemaining <= 0) continue;

    // Check if this order is involved in any bet (i.e., was matched)
    // Query bets for this event and check if this user was involved
    const betsQ = query(
      collection(db, "bets"),
      where("eventId", "==", eventId)
    );
    ...
    for (const b of betsSnap.docs) {
      const bet = b.data();
      if (bet.yesUserId === order.userId || bet.noUserId === order.userId) {
        matched = true;
        break;
      }
    }
    if (matched) continue; // Don't refund matched orders
