...
export async function placeOrder({ eventId, userId, side, price, quantity }) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    // Prevent self-matching: Check if user has orders on opposite side at matching price
    const matchingPrice = 10 - price;
    const oppositeSide = side === "yes" ? "no" : "yes";

    const selfMatchOrdersQ = query(
      ordersCol,
      where("eventId", "==", eventId),
      where("userId", "==", userId),
      where("side", "==", oppositeSide),
      where("price", "==", matchingPrice),
      where("status", "==", "open")
    );
    const selfMatchOrdersSnap = await getDocs(selfMatchOrdersQ);

    if (!selfMatchOrdersSnap.empty) {
      throw new Error(
        `You have ${oppositeSide.toUpperCase()} orders at ₹${matchingPrice}. Cannot place ${side.toUpperCase()} at ₹${price} as it would match your own order.`
      );
    }
    ...
    const matchPrice = side === "yes" ? 10 - price : price;
