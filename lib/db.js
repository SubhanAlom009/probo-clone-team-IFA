...
export async function resolveEvent(eventId, outcome, resolverId) {
  ...
  for (const bet of bets) {
    ...
    // Apply 20% commission on winnings (not on returned stake)
    const winnerStake =
      outcome === "yes" ? bet.yesLocked || 0 : bet.noLocked || 0;
    const winnings = totalPayout - winnerStake; // Profit part
    const commission = winnings * 0.2; // 20% commission on profit
    const finalPayout = totalPayout - commission; // Total minus commission
